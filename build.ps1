...
if ($Export) {
    ...
    $outputPath = Join-Path $PSScriptRoot "bin\PassedNote.iq"
    & $monkeyc -e -f $junglePath -o $outputPath -y $keyPath
} else {
    Write-Host "Building for device: $Device..." -ForegroundColor Cyan
    $outputPath = Join-Path $PSScriptRoot "bin\PassedNote.prg"
...
if ($Export) {
    Write-Host "Package Succeeded! Output: bin\PassedNote.iq" -ForegroundColor Green
} else {
    Write-Host "Build Succeeded! Output: bin\PassedNote.prg" -ForegroundColor Green
}
