...
The four faces are the "different kids' handwriting" of the Passed Note face:
  - Segoe Script Bold  -> the big time (blue ballpoint, loopy)
  - Segoe Print Bold   -> the date line (neat printing)
  - Ink Free           -> the stats lines (quick scrawl)
  - Segoe Script       -> the small notes (someone else's cursive)

Glyphs are rendered white with coverage in the alpha channel so dc.setColor() tints
the text (blue pen / black pen / red pen at draw time).
...
NOTE_CHARS = UPPER + LOWER + DIGITS + " ,.:%!?'&+-/#°"

BASE_RES = 454
# Base (454) sizes; scaled per target resolution below.
#
# note_time_lg / note_time_xl are the optional "bigger time" variants (Large /
# Huge in the TimeSize setting). Same face as note_time so they share its ascent
# ratio (ASC_TIME in NotebookView) - only the point size differs. Digits + colon
# only, so the extra atlases stay tiny (important on memory-tight MIP panels).
TIME_CHARS = DIGITS + ":"
FONTS = [
    {"name": "note_time",    "ttf": "segoescb.ttf", "size": 104, "chars": TIME_CHARS},
    {"name": "note_time_lg", "ttf": "segoescb.ttf", "size": 108, "chars": TIME_CHARS},
    {"name": "note_time_xl", "ttf": "segoescb.ttf", "size": 112, "chars": TIME_CHARS},
    {"name": "note_date",  "ttf": "segoeprb.ttf", "size": 34,  "chars": UPPER + LOWER + DIGITS + " ,.!?'-°"},
    {"name": "note_body",  "ttf": "Inkfree.ttf",  "size": 34,  "chars": NOTE_CHARS},
    {"name": "note_small", "ttf": "segoesc.ttf",  "size": 28,  "chars": NOTE_CHARS},
]

# Each target panel (width, height) and where its font set lives. Fonts are scaled
# by min(w, h) / 454. 454x454 is the base set in resources/fonts; every other
# distinct panel size used by a product in manifest.xml gets its own override
# folder, selected in monkey.jungle.
TARGETS = [
...
FONTS_XML = (
    '<resources>\n'
    '    <!-- Handwriting bitmap fonts, generated by tools/gen_fonts.py for this panel size. -->\n'
    '    <fonts>\n'
    '        <font id="NoteTime"   filename="note_time.fnt"    antialias="true" />\n'
    '        <font id="NoteTimeLg" filename="note_time_lg.fnt" antialias="true" />\n'
    '        <font id="NoteTimeXl" filename="note_time_xl.fnt" antialias="true" />\n'
    '        <font id="NoteDate"   filename="note_date.fnt"    antialias="true" />\n'
    '        <font id="NoteBody"   filename="note_body.fnt"    antialias="true" />\n'
    '        <font id="NoteSmall"  filename="note_small.fnt"   antialias="true" />\n'
    '    </fonts>\n'
    '</resources>\n'
)
...
if __name__ == "__main__":
    for w, h, reldir in TARGETS:
        ...
        for spec in FONTS:
            ...
            print("   %-11s size=%d atlas=%dx%d" % (spec["name"], s, aw, ah))
